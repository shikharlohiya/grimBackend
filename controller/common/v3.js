...
// save_for_later = require(`${path}save_for_later.js`)
...
router.get('/demo', function (req, res) {
	...
	first_endpoint.firstendpoint(req, res, function (err, data) {
		if (err) {
			res.status(500).json({ 'error': true, 'message': 'Error with endpoint .. !' })
		} else {
			res.status(200).json({ 'success': true, 'message': 'endpoint executed successfully' })
		}
	})
})
