                ...
                con.query(query, function (m_err, m_result, fields) {
                    if (m_err) {
                        ...
                        if (req.body.indent_type == "Pending") {
                            ...
                            var orderCount = `SELECT count(*) as count, (SELECT count(id) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count FROM user_orders as a where user_id in (${idsArray.join()}) AND type='indent' ${address} AND (date(created_at) >= '${req.body.from_date}') AND (date(created_at) <= '${req.body.to_date}') ${search} GROUP BY a.id HAVING indent_count > 0`
                        }
