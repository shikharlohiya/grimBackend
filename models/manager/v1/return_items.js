    ...
    mysqlPool.getConnection(function (err, con) {
        ...
        con.query(`SELECT id FROM user_details where manager_id =  ${req.body.user_id}`, function (m_err, m_result, fields) {
            if (m_err) {
                ...
                con.query(`SELECT a.id, (SELECT manager_id FROM user_details WHERE id = a.created_by ) as manager_id, a.plant_id, a.storage_location, a.indent_id, a.order_id,  a.product_id, a.order_id, (SELECT name FROM material_items WHERE material_sap_id = a.product_id ) as product_name , a.quantity, (SELECT price FROM user_indents WHERE id = a.indent_id ) as price, (SELECT total_price FROM user_indents WHERE id = a.indent_id ) as total_price, (SELECT description FROM order_status WHERE value = a.status ) as status, (SELECT color FROM order_status WHERE value = a.status ) as color, (SELECT first_name FROM user_details WHERE id = a.created_by ) as first_name, (SELECT remarks FROM order_status_logs WHERE created_by = a.created_by and status = a.status and indent_id = a.indent_id) as remarks, a.created_at, a.created_by FROM return_items as a WHERE a.status in (${req.body.status.join()}) and a.created_by in (${idsArray.join()}) `, function (err, result, fields) {
                    if (err) {
                        console.log(err);
                        res.status(500).json({
                            "success": false,
