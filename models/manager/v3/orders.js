        ...
        con.query(`SELECT id FROM user_details where manager_id =  ${req.body.user_id}`, function (m_err, m_result, fields) {
            if (m_err) {
                ...
                if (req.body.location_id.length == 0) {
                    ...
                    var orderCount = `SELECT count(*) as count, (SELECT count(id) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count FROM user_orders as a where user_id in (${idsArray.join()}) AND type='indent' AND (date(created_at) >= '${req.body.from_date}' AND date(created_at) <= '${req.body.to_date}') ${search}  GROUP BY a.id HAVING indent_count > 0 `
                    ...
                    var orderCount = `SELECT count(*) as count, (SELECT count(id) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count FROM user_orders as a where user_id in (${idsArray.join()}) AND type='indent' AND address in (${req.body.location_id.join()}) AND (date(created_at) >= '${req.body.from_date}' AND date(created_at) <= '${req.body.to_date}') ${search}  GROUP BY a.id HAVING indent_count > 0 `
                }
