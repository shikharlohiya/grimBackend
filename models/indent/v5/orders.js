...
orders.prototype.orders_func = function (req, res, callback) {
    ...
    mysqlPool.getConnection(function (err, con) {
        ...
        if (req.body.indent_type == "Pending") {
            ...
            var orderCount = `SELECT count(*) as count, (SELECT count(id) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count FROM user_orders as a where user_id = ${req.body.user_id} AND type='indent' ${address} AND (date(created_at) >= '${req.body.from_date}') AND (date(created_at) <= '${req.body.to_date}') ${search} GROUP BY a.id HAVING indent_count > 0`
        }
