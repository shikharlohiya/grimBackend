                ...
                con.query(orderCount, function (errs, results, fields) {
                    if (errs) {
                        ...
                        result.forEach((item, index) => {
                            ...
                            con.query(`SELECT id,(SELECT manager_id FROM user_details WHERE id = user_id ) as manager_id,  product_id, order_id, (SELECT name FROM material_items WHERE id = product_id ) as product_name , (SELECT material_sap_id FROM material_items WHERE id = product_id ) as product_sap_id , (SELECT quantity FROM material_stock WHERE material_id = (SELECT material_sap_id FROM material_items WHERE id = product_id ) and plant_id = '${result[index].store_address.plant_id}' and storage_loc = '${result[index].store_address.storage_location}' ) as stock , quantity, price, total_price, (SELECT description FROM order_status WHERE value = status ) as status, (SELECT color FROM order_status WHERE value = status ) as color, (SELECT first_name FROM user_details WHERE id = user_id ) as first_name,delivery_priority, remarks,reason, where_used, section, tracking_no, priority_days, created_at, created_by FROM user_indents WHERE order_id =${item.id} ${filterStatus}`, function (i_err, i_result, fields) {
                                if (i_err) {
                                    console.log(i_err);
