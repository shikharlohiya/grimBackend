...
orders.prototype.orders_func = function (req, res, callback) {
    mysqlPool.getConnection(function (err, con) {
        ...
        if (req.body.location_id.length == 0) {
            var orderQuery = `SELECT a.*, (SELECT count(1) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count ,(SELECT first_name FROM user_details WHERE id = user_id ) as first_name, (SELECT JSON_OBJECT('id', b.id, 'plant_id', b.plant_id, 'name1', b.plant_name, 'storage_location', b.storage_location)) AS 'address', (SELECT JSON_OBJECT('id', c.id, 'plant_id', c.plant_id, 'name1', c.storage_location_desc, 'storage_location', c.storage_location)) AS 'store_address' FROM user_orders AS a LEFT JOIN plant_details_sync AS b ON b.id = a.address LEFT JOIN plant_details_sync AS c ON c.id = a.plant_id where a.user_id = ${req.body.user_id} AND (date(a.created_at) >= '${req.body.from_date}' AND date(a.created_at) <= '${req.body.to_date}') HAVING indent_count > 0  ORDER BY id DESC Limit ${limit}`
            console.log(orderQuery,'---------');
            

            var orderCount = `SELECT count(*) as count FROM user_orders as a where user_id = ${req.body.user_id} AND (date(created_at) >= '${req.body.from_date}') AND (date(created_at) <= '${req.body.to_date}')  `
        } else {
            var orderQuery = `SELECT a.*, (SELECT count(1) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count, (SELECT first_name FROM user_details WHERE id = user_id ) as first_name, (SELECT JSON_OBJECT('id', b.id, 'plant_id', b.plant_id, 'name1', b.plant_name)) AS 'address', (SELECT JSON_OBJECT('id', c.id, 'plant_id', c.plant_id, 'name1', c.storage_location_desc)) AS 'store_address' FROM user_orders AS a LEFT JOIN plant_details_sync AS b ON b.id = a.address LEFT JOIN plant_details_sync AS c ON c.id = a.plant_id where a.user_id = ${req.body.user_id} and a.address in (${req.body.location_id.join()}) AND (date(a.created_at) >= '${req.body.from_date}' AND date(a.created_at) <= '${req.body.to_date}') HAVING indent_count > 0 ORDER BY id DESC Limit ${limit}`

            var orderCount = `SELECT count(*) as count , (SELECT count(1) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count FROM user_orders as a where user_id = ${req.body.user_id} AND  address in (${req.body.location_id.join()}) AND (date(created_at) >= '${req.body.from_date}' AND date(created_at) <= '${req.body.to_date}') GROUP BY a.id HAVING indent_count > 0 `
        }
                ...
                con.query(orderCount, function (errs, results, fields) {
                    if (errs) {
                        ...
                        result.forEach((item, index) => {
                            ...
                            con.query(`SELECT a.id,(SELECT manager_id FROM user_details WHERE id = a.user_id ) as manager_id, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 5) as issued_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 5 and (sto IS NOT NULL or document IS NOT NULL) ) as actual_issued_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 11) as received_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status in (14)) as return_qty,  a.product_id, a.order_id, (SELECT name FROM material_items WHERE id = a.product_id ) as product_name , (SELECT material_sap_id FROM material_items WHERE id = a.product_id ) as product_sap_id , (SELECT quantity FROM material_stock WHERE material_id = (SELECT material_sap_id FROM material_items WHERE id = a.product_id ) and plant_id = '${result[index].store_address.plant_id}' and storage_loc = '${result[index].store_address.storage_location}' ) as stock , a.quantity, a.price, a.total_price, (SELECT description FROM order_status WHERE value = a.status ) as status, (SELECT color FROM order_status WHERE value = a.status ) as color, (SELECT first_name FROM user_details WHERE id = a.user_id ) as first_name, a.delivery_priority, a.remarks,a.reason, a.where_used, a.section, a.tracking_no, a.priority_days, a.intial_qty, a.remaining_qty, a.closed, a.created_at, a.created_by, a.s_no FROM user_indents as a WHERE a.order_id =${item.id} ${filterStatus}`, function (i_err, i_result, fields) {
                                if (i_err) {
                                    console.log(i_err);
                                    con.release();
                                    res.status(500).json({
