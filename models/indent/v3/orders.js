                ...
                con.query(orderCount, function (errs, results, fields) {
                    if (errs) {
                        ...
                        result.forEach((item, index) => {
                            ...
                            con.query(`SELECT a.id, (SELECT IFNULL(sum(remaining_qty),0)-(SELECT IFNULL(sum(pr_qty),0) FROM PR_items where material_id =  b.product_id and pr_raised = '0' and status = '1' and plant_id= '${result[index].store_address.plant_id}' and store_id = '${result[index].store_address.storage_location}' ) FROM user_indents as b where product_id =  a.id and (select plant_id FROM user_orders where id = b.order_id) = '${result[index].store_address.id}') as bag, (SELECT manager_id FROM user_details WHERE id = a.user_id ) as manager_id, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 5) as issued_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 5 and (sto IS NOT NULL or document IS NOT NULL) ) as actual_issued_qty,  (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 3) as rejected_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 11) as received_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status in (14)) as return_qty,  a.product_id, a.order_id, (SELECT name FROM material_items WHERE id = a.product_id ) as product_name , (SELECT base_unit FROM material_items WHERE id = a.product_id ) as base_unit , (SELECT material_sap_id FROM material_items WHERE id = a.product_id ) as product_sap_id , (SELECT quantity FROM material_stock WHERE material_id = (SELECT material_sap_id FROM material_items WHERE id = a.product_id ) and plant_id = '${result[index].store_address.plant_id}' and storage_loc = '${result[index].store_address.storage_location}' ) as stock , a.quantity,ROUND(a.price, 2) AS price, ROUND(a.total_price, 2) AS total_price, (SELECT description FROM order_status WHERE value = a.status ) as status, (SELECT color FROM order_status WHERE value = a.status ) as color, (SELECT first_name FROM user_details WHERE id = a.user_id ) as first_name, a.delivery_priority, a.remarks,a.reason, a.where_used, a.section, a.tracking_no, a.priority_days, a.intial_qty, a.remaining_qty, a.closed, a.created_at, a.created_by, a.s_no, a.is_pr_raised FROM user_indents as a WHERE a.order_id =${item.id} ${ifilterStatus}`, function (i_err, i_result, fields) {
                                if (i_err) {
                                    console.log(i_err);
                                    con.release();
                                    res.status(500).json({
