...
purchase_request.prototype.purchase_request_latest_func = function (req, res, callback) {
    mysqlPool.getConnection(function (err, con) {
        ...
        req.body.requests.forEach((request, index) => {
            ...
            con.query(`INSERT INTO purchase_requests ( product_id,quantity, pr_quantity, wbs, created_by, status) VALUES (${request.material.id},${request.quantity},${request.pr_quantity}, '${request.wbs}', ${req.body.user_id}, 7)`, function (pr_err, pr_result) {
                if (pr_err) {
                    console.log(pr_err);
                    res.status(500).json({
                        "success": false,
                                                    ...
                                                    if (ad_err) {
                                                        ...
                                                        requestss.post({
                                                            ...
                                                            if (i == request.indents.length - 1) {
                                                                axios.post('https://grim.co.in:3002/api/v2/sync/pr_data', data)
