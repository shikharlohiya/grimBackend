                        ...
                        con.query(`SELECT count(*) as count  FROM user_orders as a WHERE (date(created_at) >= '${req.body.from_date}' AND date(created_at) <= '${req.body.to_date}') AND  type='STO' AND a.md_approval = '0' ${order_filter} ${search} `, function (errs, results, fields) {
                            if (errs) {
                                ...
                                result.forEach((item, index) => {
                                    ...
                                    con.query(`SELECT a.id, (select JSON_ARRAYAGG(order_id) FROM user_indents where id in (a.ref_indents)) as actual_indents, (SELECT manager_id FROM user_details WHERE id = a.user_id ) as manager_id, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 20) as issued_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 20 and (sto IS NOT NULL or document IS NOT NULL) ) as actual_issued_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status in (14)) as return_qty, (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 21) as received_qty,  (select COALESCE(sum(qty), 0) from order_status_logs where indent_id = a.id and status = 3) as rejected_qty,  a.product_id, a.order_id, (SELECT name FROM material_items WHERE id = a.product_id ) as product_name , (SELECT base_unit FROM material_items WHERE id = a.product_id ) as base_unit , (SELECT material_sap_id FROM material_items WHERE id = product_id ) as product_sap_id , (SELECT IFNULL( (SELECT quantity FROM material_stock WHERE material_id = (SELECT material_sap_id FROM material_items WHERE id = product_id ) and plant_id = '${result[index].store_address.plant_id}' and storage_loc = '${result[index].store_address.storage_location}' and valution_type = a.valution_type ) ,0)) as stock , a.quantity, a.price, a.total_price, (SELECT description FROM order_status WHERE value = a.status ) as status,(SELECT color FROM order_status WHERE value = a.status ) as color, (SELECT first_name FROM user_details WHERE id = a.user_id ) as first_name, a.delivery_priority, a.remarks,a.reason, a.where_used, a.section, a.tracking_no, a.priority_days, a.closed, a.closed as close, a.remaining_qty, a.intial_qty, a.created_at, a.created_by, a.s_no,(SELECT JSON_ARRAYAGG(order_id) FROM user_indents where id in (replace(replace(replace(json_unquote(a.ref_indents), ' ', ''), '[', ''), ']', ''))) as order_ids, a.valution_type FROM user_indents as a INNER JOIN material_items as b ON a.product_id=b.id WHERE a.order_id =${item.id} ${ifilterStatus}`, function (i_err, i_result, fields) {
                                        if (i_err) {
                                            console.log(i_err);
                                            ...
                                            i_result.forEach((element, i, array) => {
                                                ...
                                                con.query(`SELECT * FROM batch_ids WHERE  plant_id = '${result[index].store_address.plant_id}' and store_id = '${result[index].store_address.storage_location}' and material_id = '${element.product_sap_id}' and stock > 0; SELECT a.*, (SELECT count(1) FROM order_status_logs WHERE  FIND_IN_SET(a.serial_no,serial_numbers) as serial_reserve_count FROM serial_numbers as a WHERE  a.plant_id = '${result[index].store_address.plant_id}' and a.store_id = '${result[index].store_address.storage_location}' and a.material_id = '${element.product_sap_id}' and a.valution_type = '${element.valution_type}' and a.status='1' HAVING serial_reserve_count = 0;`, function (bserrs, bsresults, fields) {
                                                    if (bserrs) {
                                                        element.batch_ids = [];
