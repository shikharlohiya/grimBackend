...
dashboard.prototype.dashboard_func = function (req, res, callback) {
  mysqlPool.getConnection(function (err, con) {
    ...
    con.query(`SELECT id FROM user_orders where  (date(created_at) >= '${req.body.from_date}' AND date(created_at) <= '${req.body.to_date}') ORDER BY id DESC`, function (err, result, fields) {
      if (err) {
        console.log(err);
        con.release();
        res.status(500).json({
