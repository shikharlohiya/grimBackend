...
auto_pr_items.prototype.auto_pr_items_func = function (req, res, callback) {
    mysqlPool.getConnection(function (err, con) {
        ...
        ON d.id=c.order_id where a.wbs IS NOT NULL and a.status = 1 and c.status NOT IN (1,3) and a.pr_raised = '0' and d.md_approval = '0'  group by a.wbs, a.material_id`, function (err, result, fields) {
            if (err) {
                console.log(err);
                con.release();
                res.status(500).json({
                ...
                if (result.length > 0) {
                    result.forEach((item, index) => {
                        ...
                        ON b.order_id=c.id WHERE a.material_id =${item.material.id} and a.wbs =  '${item.wbs}' and a.status = 1 and b.status NOT IN (1,3) and c.md_approval = '0' and a.pr_raised = '0'`, function (u_err, u_result, fields) {
                            if (u_err) {
                                console.log(u_err);
                                con.release();
                                res.status(500).json({
