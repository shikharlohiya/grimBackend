                        ...
                        ON b.order_id=c.id WHERE a.material_id =${item.material.id} and a.wbs =  '${item.wbs}' and a.status = 1 and b.status = 2 and c.md_approval = '0'`, function (u_err, u_result, fields) {
                            if (u_err) {
                                ...
                                u_result.forEach((indent, i) => {
                                    ...
                                    con.query(`SELECT a.id, a.quantity,a.created_at, a.order_id, (SELECT description FROM order_status WHERE value = status ) as status, (SELECT color FROM order_status WHERE value = status ) as color,user_id, (SELECT first_name FROM user_details WHERE id = user_id ) as first_name, (SELECT department FROM user_details WHERE id = user_id ) as department, (SELECT remarks FROM order_status_logs WHERE indent_id = a.id and role_id = 11 and status = 2 ) as req_manager_remarks, (SELECT manager_id FROM user_details WHERE id = user_id ) as manager_id, (SELECT first_name FROM user_details WHERE id = (SELECT manager_id FROM user_details WHERE id = user_id ) ) as manager_name FROM user_indents as a WHERE product_id =${item.material.id} and id = ${indent.indent_id} `, function (in_err, in_result, fields) {
                                        if (in_err) {
                                            console.log(in_err);
                                            con.release();
