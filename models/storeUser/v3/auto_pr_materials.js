...
auto_pr_items.prototype.auto_pr_items_func = function (req, res, callback) {
    mysqlPool.getConnection(function (err, con) {
        ...
        ON d.id=c.order_id where a.wbs IS NOT NULL and a.status = '1' and c.status NOT IN (3) and a.pr_raised = '0' and d.md_approval = '0' and d.plant_id in (${storeArray.join()})  group by a.wbs, a.material_id`, function (err, result, fields) {
                        if (err) {
                            console.log(err);
                            con.release();
                            res.status(500).json({
