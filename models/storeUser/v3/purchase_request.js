...
purchase_request.prototype.purchase_request_func = function (req, res, callback) {
    mysqlPool.getConnection(function (err, con) {
        ...
        req.body.requests.forEach((request, index) => {
            ...
            con.query(`UPDATE user_indents set status = 7,updated_at = now() WHERE product_id = ${request.id} AND status = 2`, function (err, result, fields) {
                if (err) {
                    console.log(err);
                    con.release();
                    res.status(500).json({
                    ...
                    request.indents.forEach((indent, i) => {
                        con.query(`INSERT INTO order_status_logs ( indent_id, order_id, status, created_by, created_at) VALUES (${indent.id}, ${indent.order_id},7, ${req.body.user_id} , now())`, function (os_err, i_result) {
                            if (os_err) {
                                console.log(os_err);
                            } else {
                    ...
                    con.query(`INSERT INTO purchase_requests ( product_id,quantity, created_by, created_at, status) VALUES (${request.id},${request.stock}, ${req.body.user_id}, now(), 7)`, function (pr_err, pr_result) {
                        if (pr_err) {
                            console.log(pr_err);
                            con.release();
                            res.status(500).json({
...
purchase_request.prototype.purchase_request_latest_func = function (req, res, callback) {
    mysqlPool.getConnection(function (err, con) {
        ...
        req.body.requests.forEach((request, index) => {
            ...
            con.query(`SELECT id FROM order_status_logs WHERE status = 7 and indent_id in (${indent_ids.join()})`, function (os_err, os_result) {
                if (os_err) {
                    console.log(os_err);
                    con.release();
                    res.status(500).json({
                            ...
                            con.query(`UPDATE PR_items set pr_raised = '1', purchase_request_id = ${pr_result.insertId } WHERE wbs = '${request.wbs}' AND material_id = ${request.material.id} and status = 1 and indent_id in (${indent_ids.join()})`, function (err, result, fields) {
                                if (err) {
                                    ...
                                    request.indents.forEach((indent, i) => {
                                        ...
                                        if (indent.where_used == undefined) {
                                            var where_used = "";
                                        } else {
                                            var where_used = indent.where_used;
                                        }
                                        if (indent.reason == undefined) {
                                            var reason = "";
                                        } else {
                                            var reason = indent.reason;
                                        }

                                        var indent_sap = {
                                            GPRDC: pr_result.insertId.toString(),
                                            INDNO: indent.order_id.toString(),
                                            ITEMN: indent.s_no.toString(),
                                            ...
                                            POSID: wbs_no,
                                            POST1: wbs_desc,
                                            GUSRID: indent.sap_id.toString() || indent.user_id.toString(),
                                                    ...
                                                    if (indent_err) {
                                                        ...
                                                        con.query(`SELECT id,(SELECT  JSON_OBJECT('created_at', created_at, 'remarks', remarks) FROM order_status_logs where indent_id = ${indent.id} and status = 2 limit 1) as approval,  created_at, (SELECT JSON_OBJECT('id', id, 'department', department, 'manager_id', manager_id, 'email',email, 'first_name', first_name)  FROM user_details where id = (SELECT user_id FROM user_orders where id = ${indent.order_id})) as user , (SELECT JSON_OBJECT('name', name, 'base_unit',base_unit )  FROM material_items WHERE material_sap_id =  '${request.material.material_sap_id}' limit 1) as material_details, (SELECT JSON_OBJECT('id', id, 'plant_id', plant_id, 'storage_location', storage_location, 'storage_location_desc', storage_location_desc, 'plant_name', plant_name)  FROM plant_details_sync WHERE id = (SELECT address FROM user_orders where id =  ${indent.order_id})) as receiving_plant, (SELECT JSON_OBJECT('wbs_number', wbs_number, 'wbs_desc', wbs_desc)  FROM wbs_numbers WHERE wbs_number = (SELECT WBS_NO FROM user_orders where id = ${indent.order_id})) as wbs, plant_id, storage_location, storage_location_desc FROM plant_details_sync WHERE id = (SELECT plant_id FROM user_orders where id = ${indent.order_id})`, function (ad_err, ad_result) {
                                                            if (ad_err) {
                                                                ...
                                                                //     console.log(error, '---------------');
                                                                //     if (i == request.indents.length - 1) {
                                                                //         axios.post(`${process.env.host}/api/v2/sync/pr_data`, data)
                                                                //             .then(function (pr_response) {
                                                                //                 console.log('pr_response:', pr_response.data.status);
