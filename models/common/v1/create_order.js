        ...
        con.query(`INSERT INTO user_orders (user_id, plant_id, total,status, address, WBS_NO, md_approval, created_at, created_by) VALUES (${order.user_id},${order.plant.id}, ${order.total},1 , ${order.address}, '${order.WBS_NO}', '${md_approval}', now(),${order.user_id})`, function (o_err, o_result) {
          if (o_err) {
            ...
            order.items.forEach((item, index) => {
              con.query(`INSERT INTO user_indents ( product_id, user_id, order_id, quantity, price, total_price, delivery_priority, tracking_no, section, reason, where_used, status, created_at, created_by) VALUES (${item.product_id}, ${order.user_id}, ${o_result.insertId},${item.quantity},${item.price},${item.total_price},'${item.delivery_priority}','${item.tracking_no}', '${item.section}', '${item.reason}', '${item.where_used}',1 , now(),${order.user_id})`, function (i_err, i_result) {
                if (i_err) {
                  console.log(i_err);
                  con.rollback(function () {
                    res.status(500).json({
                  ...
                  con.query(`INSERT INTO order_status_logs ( indent_id, order_id, status,remarks, qty, created_by, created_at) VALUES (${i_result.insertId}, ${o_result.insertId},1, '${item.remarks}', ${item.quantity}, ${order.user_id} , now())`, function (os_err, os_result) {
                    if (os_err) {
                      console.log(os_err);
                      con.rollback(function () {
                        res.status(500).json({
                      ...
                      if ((item.stock - item.bag) - item.quantity < 0) {
                        ...
                        con.query(`INSERT INTO PR_items ( indent_id, material_id, plant_id,store_id, wbs, requested_qty, pr_qty, created_by) VALUES (${i_result.insertId}, ${item.product_id}, '${order.plant.plant_id}','${order.plant.storage_loc}', '${order.WBS_NO}', ${item.quantity},${item.quantity - bagcount},${order.user_id} )`, function (pr_err, pr_result) {
                          if (pr_err) {
                            console.log(pr_err);
                            con.rollback(function () {
                              res.status(500).json({
                      ...
                      if (index == order.items.length - 1) {
                        ...
                        con.commit(function (err) {
                          ...
                          request.post({
                            url: 'https://grim.co.in:3002/api/v2/notification_post',
                            headers: {
