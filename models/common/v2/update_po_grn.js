                                                        ...
                                                        if (req.body.status == 'PO' || req.body.status == 'GRN') {
                                                            ...
                                                            con.query(`SELECT  indent_id, order_id, qty, created_by FROM order_status_logs where indent_id = ${indent_id} and status = 7 `, function (os_err, os_result) {
                                                                if (os_err) {
                                                                    ...
                                                                    con.query(`INSERT INTO order_status_logs ( indent_id, order_id, status, qty, sap_ref_id, created_by, created_at) VALUES (${indent_id}, ${os_result[0].order_id},${status}, ${os_result[0].qty}, '${sap_ref_id}', ${os_result[0].created_by}, now())`, function (osi_err, osi_result) {
                                                                        if (osi_err) {
                                                                            console.log(osi_err);
                                                                            con.release();
                                                                            res.status(500).json({
