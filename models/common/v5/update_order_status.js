  ...
  con.getConnection(function (err, con) {
    con.beginTransaction(function (err) {
      if (err) {
        ...
        if (req.body.valution_type == "" || req.body.valution_type == "0") {
          var valution_type = "0"
          var sap_valution_type = ""
        } else {
          var valution_type = req.body.valution_type
          var sap_valution_type = req.body.valution_type
        }
        ...
        con.query(`SELECT * FROM user_indents WHERE id = ${req.body.indent_id}`, function (inerr, inresult, fields) {
          if (inerr) {
            ...
            if ((req.body.role_id == 2 || req.body.role_id == 19 || req.body.role_id == 7 || req.body.role_id == 8 || req.body.role_id == 9 || req.body.role_id == 5) && req.body.status == 12) {
            ...
            } else if ((req.body.role_id == 2 || req.body.role_id == 19 || req.body.role_id == 7 || req.body.role_id == 8 || req.body.role_id == 9) && req.body.status == 3) {
              var query = `UPDATE user_indents set status = ${req.body.status}, remaining_qty = 0, closed = 1 WHERE id = ${req.body.indent_id}`
              var role = 'Indent Manager';
            } else if ((req.body.role_id == 2 || req.body.role_id == 19 || req.body.role_id == 7 || req.body.role_id == 8 || req.body.role_id == 9) && req.body.status != 12) {
            ...
            } else if (req.body.role_id == 11) {
              if (req.body.status == 2) {
                var query = `UPDATE user_indents set status = ${req.body.status}, rm_approval = '1' WHERE id = ${req.body.indent_id}`
              } else if (req.body.status == 3) {
                var query = `UPDATE user_indents set status = ${req.body.status}, remaining_qty = 0, closed = 1 WHERE id = ${req.body.indent_id}`
              } else {
                var query = `UPDATE user_indents set status = ${req.body.status} WHERE id = ${req.body.indent_id}`
              }
              var role = 'Request Manager';
            }
            con.query(query, function (err, result, fields) {
              if (err) {
                ...
                if (req.body.status == 5 || req.body.status == 20) {
                  ...
                  if (req.body.selected_batch_id == "" || req.body.selected_batch_id == undefined) {
                    var selected_batch_id = "";
                  } else {
                    var selected_batch_id = req.body.selected_batch_id;
                    con.query(`UPDATE batch_ids set stock = stock - ${req.body.issue_qty} WHERE batch_id ='${req.body.selected_batch_id}'`, function (bn_err, sn_result) {
                      if (bn_err) {
                        console.log(bn_err);
                      }
                    })
                  ...
                  if (req.body.selected_serial_numbers == "" || req.body.selected_serial_numbers == undefined) {
                    var selected_serial_numbers = "";
                  } else {
                    var selected_serial_numbers = req.body.selected_serial_numbers.join();
                    if (req.body.selected_serial_numbers != "" && req.body.selected_serial_numbers != undefined) {
                      con.query(`UPDATE serial_numbers set status = '0' WHERE serial_no in('${req.body.selected_serial_numbers.join("','")}')`, function (sn_err, sn_result) {
                        if (sn_err) {
                          console.log(sn_err);
                        }
                      })
                  ...
                  var orderstatusLogs = `INSERT INTO order_status_logs ( indent_id, order_id, status, remarks, qty, created_by, created_at, movement_type, role_id) VALUES (${req.body.indent_id}, ${req.body.order_id},${req.body.status}, '${req.body.remarks}', ${isuue_status_qty}, ${req.body.user_id}, now(), '${req.body.movetype}', ${req.body.role_id})`
                ...
                con.query(orderstatusLogs, function (os_err, i_result) {
                  if (os_err) {
                    ...
                    if (req.body.status == 2 && req.body.role_id == 11) {
                      con.query(`SELECT a.*, (SELECT JSON_OBJECT('id', b.id, 'plant_id', b.plant_id, 'name1', b.plant_name, 'storage_location', b.storage_location, 'storage_location_desc', b.storage_location_desc)) AS 'address', (SELECT JSON_OBJECT('id', c.id, 'plant_id', c.plant_id, 'name1', c.storage_location_desc, 'storage_location_desc', c.storage_location_desc, 'storage_location', c.storage_location)) AS 'store_address' FROM user_orders AS a LEFT JOIN plant_details_sync AS b ON b.id = a.address LEFT JOIN plant_details_sync AS c ON c.id = a.plant_id WHERE a.id = ${req.body.order_id}`, function (gi_err, gi_result) {
                        if (gi_err) {
                          ...
                          gi_result[0].store_address = JSON.parse(gi_result[0].store_address)
                          gi_result[0].address = JSON.parse(gi_result[0].address)

                          con.query(`SELECT IFNULL(sum(a.qty),0) as bag, (SELECT quantity FROM material_stock WHERE plant_id = '${gi_result[0].store_address.plant_id}' AND storage_loc = '${gi_result[0].store_address.storage_location}' AND material_id = ${req.body.product_id} AND valution_type = '${valution_type}') as stock, (SELECT id FROM material_items WHERE material_sap_id = ${req.body.product_id}) as material_id FROM stock_reserve as a where a.material_id = ${req.body.product_id} and a.plant_id = ${gi_result[0].store_address.id} AND a.valution_type = '${valution_type}'`, function (b_err, b_result) {
                            if (b_err) {
                              console.log(b_err);
                              con.rollback(function () {
                                res.status(500).json({
                              ...
                              if ((b_result[0].stock - b_result[0].bag) - req.body.quantity < 0) {
                                ...
                                con.query(`INSERT INTO PR_items ( indent_id, material_id, plant_id,store_id, wbs, requested_qty, pr_qty, status, created_by) VALUES (${req.body.indent_id}, ${b_result[0].material_id}, '${gi_result[0].store_address.plant_id}','${gi_result[0].store_address.storage_location}', '${gi_result[0].WBS_NO}', ${req.body.quantity},${req.body.quantity - availableQty}, '1', ${req.body.indentUser_id} )`, function (pr_err, pr_result) {
                                  if (pr_err) {
                                    console.log(pr_err);
                                    con.rollback(function () {
                                      res.status(500).json({
                                    ...
                                    con.query(`INSERT INTO stock_reserve (indent_id, order_id, material_id, qty, plant_id, created_by, valution_type) VALUES (${req.body.indent_id},${req.body.order_id}, '${req.body.product_id}', ${availableQty}, '${gi_result[0].store_address.id}', ${req.body.user_id}, '${valution_type}')`, function (ri_err, ri_result) {
                                      if (ri_err) {
                                        console.log(ri_err);
                                        con.rollback(function () {
                                ...
                                con.query(`INSERT INTO stock_reserve (indent_id, order_id, material_id, qty, plant_id, created_by, valution_type) VALUES (${req.body.indent_id},${req.body.order_id}, '${req.body.product_id}', ${req.body.quantity}, '${gi_result[0].store_address.id}', ${req.body.user_id}, '${valution_type}')`, function (ri_err, ri_result) {
                                  if (ri_err) {
                                    console.log(ri_err);
                                    con.rollback(function () {
                              ...
                              con.query(`SELECT id, created_at, (SELECT JSON_OBJECT('id', id, 'department', department, 'manager_id', manager_id, 'email',email, 'first_name', first_name)  FROM user_details where id = (SELECT user_id FROM user_orders where id = ${req.body.order_id})) as user , (SELECT JSON_OBJECT('name', name, 'base_unit',base_unit )  FROM material_items WHERE material_sap_id =  '${req.body.product_id}' limit 1) as material_details, (SELECT JSON_OBJECT('id', id, 'plant_id', plant_id, 'storage_location', storage_location, 'storage_location_desc', storage_location_desc, 'plant_name', plant_name)  FROM plant_details_sync WHERE id = (SELECT address FROM user_orders where id =  ${req.body.order_id})) as receiving_plant, (SELECT JSON_OBJECT('wbs_number', wbs_number, 'wbs_desc', wbs_desc)  FROM wbs_numbers WHERE wbs_number = (SELECT WBS_NO FROM user_orders where id = ${req.body.order_id})) as wbs, plant_id, storage_location, storage_location_desc FROM plant_details_sync WHERE id = (SELECT plant_id FROM user_orders where id = ${req.body.order_id})`, function (ad_err, ad_result) {
                                if (ad_err) {
                                  ...
                                  con.query(`SELECT a.first_name, a.email, (SELECT role from user_roles where id = a.role_id) FROM user_details as a where a.id =${req.body.user_id}`, function (mn_err, mn_result) {
                                    if (mn_err) {
                                      console.log(mn_err);
                                    } else {
                                      ...
                                      var substitutions = {
                                        ...
                                        "subject": `Indent ${req.body.order_id} is Approved by ${mn_result[0].first_name}`,
                                        "template": "indentApprovedTemp",
                                        "view": {
                                          "subjectHeader": `Indent ${req.body.order_id}  is approved by ${mn_result[0].first_name}`,
                    ...
                    } else if (req.body.status == 2 && req.body.role_id != 11) {

                      con.query(`UPDATE Indent_approvals set finish = '1' where indent_id = ${req.body.indent_id} and approver_id = ${req.body.user_id}`, function (ia_err, ia_result, fields) {
                        if (ia_err) {
                          console.log(ia_err);
                          con.rollback(function () {
                            res.status(500).json({
                            ...
                            if (ad_err) {
                              ...
                              con.query(`SELECT a.first_name, a.email, (SELECT role from user_roles where id = a.role_id) FROM user_details as a where a.id =${req.body.user_id}`, function (mn_err, mn_result) {
                                if (mn_err) {
                                  ...
                                  con.query(`select *,(select first_name from user_details where id = approver_id) as approver_name from Indent_approvals where id > (select COALESCE(MAX(id),0) from Indent_approvals where role_id = ${req.body.role_id} and indent_id = ${req.body.indent_id}) limit 1;`, function (ap_err, ap_result) {
                                    if (ap_err) {
                                      console.log(ap_err);
                                    }
                                    if (ap_result.length > 0) {
                                      if (ap_result[0].indent_id == req.body.indent_id) {

                                        var appSubstitutions = {
                                          "from": "IBGroup <grim@ibgroup.co.in>",
                                          "to": ap_result[0].approver_email,
                                          "subject": `Indent ${req.body.order_id} is waiting for your Approval`,
                                          "template": "indentApprovedTemp",
                                          "view": {
                                            "subjectHeader": `Indent ${req.body.order_id}  is approved by ${mn_result[0].first_name} and has been sent to you for further 
                                                action.`,
                                            "first_name": ap_result[0].approver_name,
                                            "indent_id": req.body.order_id,
                                            "raised_by": ad_result[0].user.first_name,
                                            "indent_date": req.body.created_at,
                                            "item_name": ad_result[0].material_details.name,
                                        ...
                                        axios.post(`${process.env.host}/api/v4/sendmail`, appSubstitutions)
                                          .then(function (response) {
                                            // console.log(response);
                                          }).catch(function (error) {
                                            // handle error
                                        ...
                                        con.query(`UPDATE user_indents set approval_finish = '1'  where id = ${req.body.indent_id}`, function (ai_err, ai_result, fields) {
                                          if (ai_err) {
                                            console.log(ai_err);
                                          }
                                        })
                      ...
                      con.query(`SELECT id,(SELECT  JSON_OBJECT('created_at', created_at, 'remarks', remarks) FROM order_status_logs where indent_id = ${req.body.indent_id} and status = 2 limit 1) as approval,  created_at, (SELECT JSON_OBJECT('id', id, 'department', department, 'manager_id', manager_id, 'email',email, 'first_name', first_name)  FROM user_details where id = (SELECT user_id FROM user_orders where id = ${req.body.order_id})) as user , (SELECT JSON_OBJECT('name', name, 'base_unit',base_unit )  FROM material_items WHERE material_sap_id =  '${req.body.product_id}' limit 1) as material_details, (SELECT JSON_OBJECT('where_used', where_used)  FROM user_indents WHERE id =  '${req.body.indent_id}') as indent_details, (SELECT JSON_OBJECT('id', id, 'plant_id', plant_id, 'storage_location', storage_location, 'storage_location_desc', storage_location_desc, 'plant_name', plant_name)  FROM plant_details_sync WHERE id = (SELECT address FROM user_orders where id =  ${req.body.order_id})) as receiving_plant, (SELECT JSON_OBJECT('wbs_number', wbs_number, 'wbs_desc', wbs_desc)  FROM wbs_numbers WHERE wbs_number = (SELECT WBS_NO FROM user_orders where id = ${req.body.order_id})) as wbs, plant_id, storage_location, storage_location_desc FROM plant_details_sync WHERE id = (SELECT plant_id FROM user_orders where id = ${req.body.order_id})`, function (ad_err, ad_result) {
                        if (ad_err) {
                          ...
                          if (req.body.closed == 1) {
                            ...
                            con.query(`UPDATE stock_reserve set qty = qty - ${req.body.issue_qty} where indent_id = ${req.body.indent_id}`, function (sr_err, sr_result, fields) {
                              if (sr_err) {
                                console.log(sr_err);
                              }
                            })
                            ...
                            if (mn_err) {
                              ...
                              var data = {
                                indent_data: {
                                  ...
                                  BWTAR: sap_valution_type,
                                  SERNR: selected_serial_numbers,
                                  CHARG: selected_batch_id
                                }
                              }
                              ...
                              axios.post(`${process.env.host}/api/${process.env.sap_version}/sync/indent_data`, data)
                                .then(function (response) {
                                  ...
                                  console.log(`UPDATE material_stock set quantity = quantity - ${req.body.issue_qty}  WHERE material_id = ${req.body.product_id} AND plant_id = '${ad_result[0].plant_id}' AND storage_loc = '${ad_result[0].storage_location}' and valution_type = '${valution_type}'`, '-----------------');

                                  con.query(`UPDATE material_stock set quantity = quantity - ${req.body.issue_qty}  WHERE material_id = ${req.body.product_id} AND plant_id = '${ad_result[0].plant_id}' AND storage_loc = '${ad_result[0].storage_location}' and valution_type = '${valution_type}'`, function (su_err, su_result) {
                                    if (su_err) {
                                      console.log(su_err);
                                      con.rollback(function () {
                              ...
                              con.query(osrQuery, function (d_err, d_result) {
                                if (d_err) {
                                  ...
                                  d_result.forEach(function (disrow, index) {
                                    ...
                                    if (disrow.batch_ids == null || disrow.batch_ids == "" || disrow.batch_ids == undefined) {
                                      var batch_ids = ""
                                    } else {
                                      var batch_ids = disrow.batch_ids
                                    }

                                    if (disrow.serial_numbers == null || disrow.serial_numbers == "" || disrow.serial_numbers == undefined) {
                                      var serial_numbers = ""
                                    } else {
                                      var serial_numbers = disrow.serial_numbers
                                    }
                                    var edata = {
                                      INDNO: req.body.order_id.toString(),
                                      ITEMN: req.body.s_no.toString(),
                              ...
                              if (ad_err) {
                                ...
                                con.query(`SELECT first_name FROM user_details where id =${ad_result[0].user.manager_id}`, function (mn_err, mn_result) {
                                  if (mn_err) {
                                    ...
                                    if (ad_result[0].sto == '' || ad_result[0].sto == null) {
                                      var sto = "";

                                    } else {
                                      var sto = ad_result[0].sto;
                          ...
                          con.query(`SELECT a.first_name, a.email, (SELECT role from user_roles where id = a.role_id) FROM user_details as a where a.id =${req.body.user_id}`, function (mn_err, mn_result) {
                            if (mn_err) {
                              ...
                              var substitutions = {
                                "from": "IBGroup <grim@ibgroup.co.in>",
                                "to": ad_result[0].user.email,
                                "subject": `Indent ${req.body.order_id} is Hold by ${mn_result[0].first_name}`,
                                "template": "indentHoldTemp",
                                "view": {
                                  "subjectHeader": `Indent ${req.body.order_id}  is hold by ${mn_result[0].first_name}`,
                                  "first_name": ad_result[0].user.first_name,
                                  "indent_id": req.body.order_id,
                                  "raised_by": ad_result[0].user.first_name,
                                  "indent_date": req.body.created_at,
