...
orders.prototype.orders_func = function (req, res, callback) {
    ...
    if (req.body.indent_type == "Pending") {
        ...
        var orderQuery = `SELECT a.*,  (SELECT count(1) FROM Indent_approvals WHERE order_id =a.id) as approval_count, (SELECT count(1) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count,  (SELECT first_name FROM user_details WHERE id = user_id ) as first_name, (SELECT JSON_OBJECT('id', b.id, 'plant_id', b.plant_id, 'name1', b.plant_name, 'storage_location', b.storage_location, 'storage_location_desc', b.storage_location_desc)) AS 'address', (SELECT JSON_OBJECT('id', c.id, 'plant_id', c.plant_id, 'name1', c.storage_location_desc, 'storage_location_desc', c.storage_location_desc, 'storage_location', c.storage_location)) AS 'store_address' FROM user_orders AS a LEFT JOIN plant_details_sync AS b ON b.id = a.address LEFT JOIN plant_details_sync AS c ON c.id = a.plant_id where a.type='indent'  ${address} AND (date(a.created_at) >= '${req.body.from_date}' AND date(a.created_at) <= '${req.body.to_date}') ${search}  HAVING indent_count > 0 and approval_count > 0 ORDER BY id DESC Limit ${limit}`

        var orderCount = `SELECT count(*) as count,  (SELECT count(1) FROM Indent_approvals WHERE order_id =a.id) as approval_count, (SELECT count(id) FROM user_indents WHERE order_id =a.id ${filterStatus}) as indent_count FROM user_orders as a where  type='indent' ${address} AND (date(created_at) >= '${req.body.from_date}' AND date(created_at) <= '${req.body.to_date}') ${search}  GROUP BY a.id HAVING indent_count > 0 and approval_count > 0 `
    }
